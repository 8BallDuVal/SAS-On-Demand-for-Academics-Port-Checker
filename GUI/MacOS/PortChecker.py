...
#-------------------------- Logic to output results to GUI window after Ports are checked ------------------------------------#       
    ...
    #-------- Port Open/Closed results from scanports() function being output to GUI textbox (self.output_textbox) ---------------#
        ...
        #----------- looping through the keys/values in scanports_dict to see if the value contains OPEN or CLOSED ----#
            if 'OPEN' in value:
                results.append(value+ ' (successfully connected to '+key+')')                           # Formatting the results that will be displayed, appending them in a list
            elif 'CLOSED' in value:
                closed.append(value+ ' (did not successfully connect to '+key+
                                        '. You may need to unblock this port to connect.)')             # Formatting the results that will be displayed, appending them in a list
        #-------------- labelling the (US ONLY) servers since there are two extra servers for the US ------------------------------#
            elif 'odaws03.oda.sas.com' in key or 'odaws04.oda.sas.com' in key:
                if 'OPEN' in value:
                    results.append(value+ ' (successfully connected to '+key+' (US ONLY))')             # Formatting the results that will be displayed, appending them in a list
                elif 'CLOSED' in value:
                    closed.append(value+ ' (did not successfully connect to '+key+
                                         ' (US ONLY). You may need to unblock this port to connect.)')  # Formatting the results that will be displayed, appending them in a list

        #-------------------- OUTPUT THIS MESSAGE TO OUTPUT TEXTBOX IF ALL PORTS ARE OPEN: ----------------------------------#
        if len(closed) == 0:
            self.output_textbox.insert(END,'Congratulations! The SAS OnDemand Port Checker did not detect'+     # Outputting this text to the GUI window
        ...
        #----------------------- IF ANY OF THE PORTS ARE CLOSED, OUTPUT THIS MESSAGE TO OUTPUT TEXTBOX -----------------------#
            ...
            self.output_textbox.insert(END,'Unfortunately, we discovered that there were some network ports being'+ # Outputting this text to the GUI window
